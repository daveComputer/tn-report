...
\begin{enumerate}
    \item \textbf{La standardisation des compétences (Volet 1 - Le Quiz) :} Pour répondre aux attentes des clients, les compétences techniques des consultants doivent être solides et certifiées. Oxyl prépare ses recrues aux certifications Oracle (\textit{OCA} et \textit{OCP Java SE}~\cite{boyarsky2020ocp}) et \textit{AWS Certified Cloud Practitioner}. Pour faciliter cette préparation, nous avons développé une plateforme web sur-mesure en Spring Core~\cite{walls2022spring} et Angular. Elle sert à la fois de projet pratique d'apprentissage pour les stagiaires et d'outil quotidien d'entraînement et de suivi pédagogique pour l'entreprise.
...
\begin{itemize}
    ...
    \item \textbf{L'Excellence :} maintenir des standards élevés de qualité de code (\textit{Clean Code}~\cite{martin2008clean}, architecture modulaire, tests) ;
...
\begin{tikzpicture}[
    ...
    phasebox/.style={draw=blue!80!black, fill=blue!6, rounded corners=3pt, align=center, inner sep=5pt, line width=0.7pt, text width=3.3cm, minimum height=1.3cm},
    detailbox/.style={draw=gray!50, fill=gray!5, rounded corners=2pt, align=left, inner sep=4pt, font=\sffamily\tiny, text width=3.3cm},
    certbox/.style={draw=teal!80!black, fill=teal!8, rounded corners=2pt, align=center, inner sep=4pt, font=\sffamily\bfseries\tiny, text width=3.3cm},
    ...
    \node[phasebox, fill=teal!6, draw=teal!80!black] (p2) at (4.5, 0) {\textbf{\normalsize 2. PFE \& Formation}\\[1pt]\textit{3 mois intensifs}};
    ...
    \node[phasebox, fill=orange!8, draw=orange!80!black, text width=3.2cm] (p3a) at (9.0, 0.9) {\textbf{\small Parcours en Régie}\\[1pt]\textit{Immersion Client}};
    ...
    \node[phasebox, fill=purple!8, draw=purple!80!black, text width=3.2cm] (p3b) at (9.0, -1.1) {\textbf{\small Pôle R\&D / Usine IA}\\[1pt]\textit{Projets Internes}};
    ...
    % Connecteurs
    \draw[patharrow] (p1.east) -- (p2.west);
    \draw[thick, draw=teal!80!black] (p2.east) -- ++(0.5, 0) coordinate (fork);
    \draw[forkarrow] (fork) |- (p3a.west);
    \draw[forkarrow] (fork) |- (p3b.west);
...
\begin{tikzpicture}[
    ...
    \node[pillarbox, draw=purple!80!black, fill=purple!5] (technique) at (3.3, 0) {
    ...
    \path (humain.south) -- (technique.south) coordinate[midway] (mid);
    \node[synergybox, anchor=north] (synergie) at ([yshift=-0.9cm]mid) {
    ...
    % Flèches
    \draw[arrowstyle, draw=blue!80!black] (humain.south) -- (humain.south |- synergie.north);
    \draw[arrowstyle, draw=purple!80!black] (technique.south) -- (technique.south |- synergie.north);
...
Le backend est structuré en couches distinctes selon les principes de responsabilité unique (\textit{Single Responsibility Principle}), d'inversion de dépendances et de \textit{Clean Architecture}~\cite{martin2017cleanarch} (Figure~\ref{fig:architecture_globale_quiz}).
...
\begin{tikzpicture}[
    ...
    % TIER 1 : FRONTEND
    \node[tierbox, fill=teal!4, draw=teal!80!black, text width=13.2cm] (tier_front) at (0, 0) {
        \textbf{\color{teal!80!black}\normalsize TIER 1 : Frontend Single Page Application (Angular 17)}\\[4pt]
        \centering
        \begin{tikzpicture}[node distance=0.4cm]
            \node[compbox, fill=teal!10, draw=teal!70!black, text width=3.5cm] (c1) at (-4.4, 0) {\textbf{Composants UI}\\Quiz, Dashboard, Admin};
            \node[compbox, fill=teal!10, draw=teal!70!black, text width=3.8cm] (c2) at (0, 0) {\textbf{Services TypeScript}\\Gestion d'état, RxJS};
            \node[compbox, fill=teal!10, draw=teal!70!black, text width=3.5cm] (c3) at (4.4, 0) {\textbf{HTTP Interceptor}\\Cookies JWT \& Erreurs};
            \draw[-{Latex}, thick, draw=teal!70!black] (c1) -- (c2);
            \draw[-{Latex}, thick, draw=teal!70!black] (c2) -- (c3);
        \end{tikzpicture}
    };

    % TIER 2 : BACKEND SPRING
    \node[tierbox, text width=13.2cm, below=1.2cm of tier_front] (tier_back) {
        \textbf{\color{blue!80!black}\normalsize TIER 2 : API REST Backend (Java / Spring Framework)}\\[4pt]
        \centering
        \begin{tikzpicture}
            \node[compbox, fill=blue!10, text width=3.6cm] (b1) at (-4.4, 0) {\textbf{Contrôleurs REST}\\{\ttfamily\tiny ChapterController}\\{\ttfamily\tiny SerieController}\\{\ttfamily\tiny UserController}};
            \node[compbox, fill=purple!10, draw=purple!70!black, text width=4.1cm] (b2) at (0, 0) {\textbf{Services Métier}\\{\ttfamily\tiny SerieService} (Tirage)\\{\ttfamily\tiny ChapterService} (Path)\\{\ttfamily\tiny ScoreCalculatorService}};
            \node[compbox, fill=blue!10, text width=3.6cm] (b3) at (4.4, 0) {\textbf{Couche Persistance}\\{\ttfamily\tiny ChapterRepository}\\{\ttfamily\tiny SerieRepository}\\{\ttfamily\tiny UserRepository}};
    ...
    % TIER 3 : DATABASE
    \node[tierbox, fill=orange!4, draw=orange!80!black, text width=13.2cm, below=1.2cm of tier_back] (tier_db) {
        \textbf{\color{orange!80!black}\normalsize TIER 3 : Base de Données Relationnelle}\\[4pt]
        \centering
        \begin{tikzpicture}
            \node[compbox, fill=orange!10, draw=orange!70!black, text width=5.8cm] (db1) at (-3.4, 0) {\textbf{Schéma MySQL / H2}\\Tables {\ttfamily\tiny chapter}, {\ttfamily\tiny question}, {\ttfamily\tiny answer}, {\ttfamily\tiny serie}, {\ttfamily\tiny user}};
            \node[compbox, fill=orange!10, draw=orange!70!black, text width=5.8cm] (db2) at (3.4, 0) {\textbf{Index \& Optimisations}\\Index sur {\ttfamily\tiny parent\_path} et suppression logique};
        \end{tikzpicture}
    };

    % Connecteurs
    \draw[busarrow, draw=teal!80!black] (tier_front.south) -- node[fill=white, inner sep=3pt, font=\sffamily\tiny\bfseries, text=teal!80!black] {Échanges REST (JSON) / Cookies HttpOnly} (tier_back.north);
    \draw[busarrow, draw=orange!80!black] (tier_back.south) -- node[fill=white, inner sep=3pt, font=\sffamily\tiny\bfseries, text=orange!80!black] {Spring Data JPA / Hibernate} (tier_db.north);
...
Pour évaluer les agents sur des cas réels, le benchmark \textbf{SWE-bench}~\cite{jimenez2023swebench} et les interfaces agentiques comme \textbf{SWE-agent}~\cite{yang2024sweagent} se sont imposés comme des références. Ils testent la capacité d'un agent à résoudre de vrais tickets issus de projets open-source en modifiant le code jusqu'à validation de la suite de tests.
...
\begin{itemize}
    ...
    \item \textbf{OpenAI Codex / CLI}~\cite{chen2021codex} : pour l'édition ciblée de fichiers de code ;
    \item \textbf{Claude Code CLI}~\cite{anthropic2024claude} : pour son raisonnement sur de larges fenêtres de contexte et son exécution fluide de commandes terminal.
\end{itemize}
...
Pour que l'usine reste pérenne et puisse changer de modèle sans réécrire l'orchestrateur, nous avons structuré le code autour de patrons de conception (\textit{Design Patterns}~\cite{gamma1994design}) classiques.
