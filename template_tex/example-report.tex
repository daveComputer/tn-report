...
\small
\begin{tabular}{|p{3.2cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|p{3.2cm}|}
\hline
...
\begin{tikzpicture}[
    node distance=1.8cm,
    interface/.style={draw, rectangle, fill=blue!10, rounded corners, minimum width=5.2cm, minimum height=1.3cm, align=center, font=\small},
    adapter/.style={draw, rectangle, fill=green!10, minimum width=3.8cm, minimum height=1.4cm, align=center, font=\footnotesize},
    resolver/.style={draw, rectangle, fill=orange!10, dashed, minimum width=4.8cm, minimum height=1.2cm, align=center, font=\small},
    arrow/.style={-{Latex[length=3mm]}, thick},
    implements/.style={-{Triangle[open, length=3mm]}, dashed, thick}
]
    % Nodes
    \node[interface] (provider) {\textbf{<<interface>>}\\ \textbf{LLMProvider}\\ \scriptsize + invokeAgent(step: Step): Promise<ExecutionResult>\\ + formatPrompt(context, prompt): string};
    
    \node[adapter, below left=1.5cm and -0.8cm of provider] (claude) {\textbf{ClaudeCodeAdapter}\\ \scriptsize - apiKey: string\\ + invokeAgent()\\ + spawnCliProcess()};
    
    \node[adapter, below right=1.5cm and -0.8cm of provider] (antigravity) {\textbf{AntigravityAdapter}\\ \scriptsize - oauthSession: OAuthToken\\ + invokeAgent()\\ + initSession()};
    
    \node[resolver, above=1.2cm of provider] (resolver) {\textbf{LLMProviderResolver}\\ \scriptsize - instance: LLMProvider | null\\ + createLLMProvider(config): void\\ + getLLMProvider(): LLMProvider};

    % Links
    \draw[implements] (claude) -- (provider.south west);
    \draw[implements] (antigravity) -- (provider.south east);
    \draw[arrow] (resolver) -- node[right, font=\scriptsize] {Instancie / Retourne} (provider);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    comporange/.style={draw=orange!90!black, fill=white, rounded corners=3pt, minimum width=3.0cm, minimum height=0.85cm, align=center, font=\scriptsize},
    compteal/.style={draw=teal!90!black, fill=white, rounded corners=3pt, minimum width=3.0cm, minimum height=0.85cm, align=center, font=\scriptsize},
    agentbox/.style={draw=purple!80!black, fill=purple!12, rounded corners=4pt, minimum width=7.2cm, minimum height=1.1cm, align=center, font=\scriptsize},
    guardbox/.style={draw=red!70!black, fill=red!10, rounded corners=3pt, minimum width=3.0cm, minimum height=0.85cm, align=center, font=\scriptsize},
    scriptbox/.style={draw=blue!80!black, fill=blue!12, rounded corners=4pt, minimum width=6.8cm, minimum height=0.9cm, align=center, font=\scriptsize\bfseries}
]

    % =========================================================================
    % 1. ZONE FORGES & BACKLOGS (EXTERNE)
    % =========================================================================
    ...
    % =========================================================================
    % 2. ZONE SUPERVISEUR (SUPERVISOR V5)
    % =========================================================================
    ...
    \node[comporange] (backlog_prov) at (-3.9, 2.3) {\textbf{BacklogProvider}\\ \tiny fetchReadyTickets() / claimTicket()};
    \node[comporange, fill=orange!15] (resolver) at (0, 2.3) {\textbf{ProviderResolver}\\ \tiny Registry / Singleton config};
    \node[comporange] (in_flight) at (3.9, 2.3) {\textbf{In-Flight Store}\\ \tiny Suivi PIDs, Heartbeat \& Survivors};

    % =========================================================================
    % 3. ZONE WORKSPACE WORKTREE & WORKER (LAUNCHER V5)
    % =========================================================================
    ...
    \node[compteal, fill=gray!12] (git_local) at (-3.9, -0.6) {\textbf{Git Worktree Local}\\ \tiny \code{.worktrees/<branch>} + \code{.task/}};
    ...
    \node[guardbox] (guards) at (3.9, -0.6) {\textbf{Garde-fous}\\ \tiny validateMechanical / Linters};
    ...
    % =========================================================================
    % 4. PASSERELLE DE FINALISATION DÉTERMINISTE
    % =========================================================================
    \node[scriptbox] (mr_creator) at (0, -4.4) {Script Déterministe \code{create-mr.ts}\\ \tiny CodeBaseProvider.handleMergeRequest() avec tokens typés (\code{brandAs})};

    % =========================================================================
    % FLÈCHES ET FLUX D'ORCHESTRATION
    % =========================================================================
    % 1. Forges <-> Superviseur
    ...
    % 3. Intérieur du Worktree
    \draw[->, thick, draw=teal!80!black] (runner) -- node[above=1pt, font=\tiny] {Init/Commit} (git_local);
    \draw[<->, thick, draw=purple!80!black] (runner.south) -- node[right=2pt, font=\tiny] {3. Invocations (stdin/stdout)} (llm_agent.north);
    \draw[<->, thick, draw=gray!70!black] (llm_agent.west) -| (-3.9, -1.4) -- node[left=1pt, font=\tiny] {Édition} (git_local.south);
    \draw[->, thick, draw=teal!80!black] (runner) -- node[above=1pt, font=\tiny] {4. Contrôle} (guards);

    % 4. Finalisation vers create-mr
    \draw[->, thick, draw=teal!80!black] (guards.south) -- (3.9, -4.4) -- node[above=1pt, font=\tiny] {5. Succès gardes} (mr_creator.east);

    % 5. create-mr vers Forges (Remontée à l'extérieur gauche)
